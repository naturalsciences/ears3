...
public class StagedRdfService {
    ...
    @Value("${ears.ontology.rdf.live-path}")
    private String livePathStr;
    ...
    /** The one deliberate "publish"/"activate" step. Archives the outgoing live file first. */
    public void activate() throws IOException {
        if (!Files.exists(staged())) {
            throw new IllegalStateException("Nothing staged - upload or generate a file before activating.");
        }
        Files.createDirectories(archiveDir());
        if (Files.exists(live())) {
            String ts = OffsetDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
            Files.copy(live(), archiveDir().resolve("live_" + ts + ".rdf"), StandardCopyOption.REPLACE_EXISTING);
        }
    ...
    public InputStream openLive() throws IOException {
        return Files.newInputStream(live());
    }

    /** Filename to use for Content-Disposition when serving the live file for download. */
    public String liveFilename() {
        return live().getFileName().toString();
    }
     ...
     */
    public OntologyHeaderReader.OntologyHeader liveHeader() throws IOException {
        try (InputStream in = openLive()) {
