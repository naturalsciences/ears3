...
public abstract interface ProjectRepository
    ...
    @Query("select p from Project p left join p.term l where l.identifier= ?1 or l.urn=?1")
    public abstract Project findByIdentifier(String identifier);

    @Modifying
    @org.springframework.transaction.annotation.Transactional
    @Query("delete from Project p where p.term in (select l from LinkedDataTerm l where l.identifier= ?1 or l.urn=?1)")
    public abstract void deleteByIdentifier(String identifier);
        
    @Query("select p from Project p left join p.term l where l.identifier in (?1)")
    public abstract List<Project> findAllByIdentifier(Set<String> identifiers);
       
    @Query("select p from Project p left join p.term l where l.name= ?1")
