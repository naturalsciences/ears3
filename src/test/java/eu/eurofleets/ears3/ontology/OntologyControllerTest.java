...
public class OntologyControllerTest {
    ...
    private ResultActions uploadVesselFile(MockMultipartFile multipartFile, String user, String pass, int status)
            throws Exception {
        return this.mockMvc
                .perform(multipart("/ontology/vessel/upload").file(multipartFile).accept(Constants.APPLICATION_XML_UTF8).header(
    ...
    private ResultActions uploadProgramFile(MockMultipartFile multipartFile, int status) throws Exception {
        return this.mockMvc.perform(multipart("/ontology/program/upload").file(multipartFile).accept(Constants.APPLICATION_XML_UTF8))
                .andDo(print())
                .andExpect(status().is(status));
    }
    ...
    public void testSparqlEndpoint() throws Exception {
        ...
        this.mockMvc.perform(
                        get("/ontology/vessel/sparql?q=" + q).headers(httpHeaders))
                //.andDo(print())
                .andExpect(status().isOk());
    ...
    public void testSparqlEndpointProgram() throws Exception {
        ...
        q = UriUtils.encode(q, "UTF8"); //encoding no longer needed. Something autoencodes.
        this.mockMvc.perform(
                        get("/ontology/vessel/sparql?q=" + q).headers(httpHeaders))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.results.bindings.length()").value(80));

        this.mockMvc.perform(get("/ontology/vessel/sparql?program=program-abc&q=" + q)
                        .headers(httpHeaders))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.results.bindings.length()").value(80));
